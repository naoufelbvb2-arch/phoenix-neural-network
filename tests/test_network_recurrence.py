...
def test_loop_reverberates_with_convergent_input() -> None:
    ...
    Measured: ONE injection at t=0 yields 108 spikes in a perfectly regular
    alternating train (A@1, B@3, A@5, B@7, ...) — a 4 ms period, 500 Hz
    aggregate — versus a feed-forward baseline of just 2 spikes.
    """
    net = _network(A, B)
    ...
    assert any(t > 100.0 for _, t in train)

    # 4) The loop alternates cleanly at a 4 ms period (2 ms per hop).
    assert train[:6] == [(A, 1.0), (B, 3.0), (A, 5.0), (B, 7.0), (A, 9.0), (B, 11.0)]

    # --- MEASURED PROPERTY, pinned honestly: the loop EXTINGUISHES ITSELF ---
    # Raw STDP under recurrence is net-DEPRESSING here: each synapse is both
    # potentiated (its post fires after its pre) and depressed (its pre fires
    # again after its post, one cycle later), and depression wins. The weights
    # decay 15.0 -> ~12.84, and the sustain threshold is ~12.82 (two bumps of w
    # must still clear 25 mV after one tick of leak). Once they cross it, the
    # loop can no longer re-ignite and falls silent at t=215.
    assert max(t for _, t in train) == 215.0
    assert not any(t > 250.0 for _, t in train)
    final_ab = [s.weight for s in net.outgoing[A]]
    final_ba = [s.weight for s in net.outgoing[B]]
    assert all(w < 15.0 for w in final_ab + final_ba)  # net depression
    assert final_ab[0] == pytest.approx(12.835, abs=0.01)
    assert final_ba[0] == pytest.approx(12.780, abs=0.01)
